...
export default function UserPhotos(props:any){

    const [userPhotos,setUserPhotos] = React.useState<data>()
    
    React.useEffect(() => {
        axios.get('https://api.unsplash.com//users/' + props.username + '/photos',{
    ...
    <>
        <Container maxWidth={false} sx={{ paddingTop: '2rem' }}>
            ...
            {!isEmpty(userPhotos) && userPhotos.map((item:any) => (
